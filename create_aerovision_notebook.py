...
def generate_notebook(stage_num):
    ...
    if stage_num == 'master':
        ...
        add_code([
            ...
            "        ",
            "        # [EXPERIMENTAL] Train lazy learners on CNN Embeddings",
            "        print(f'  [EXPERIMENTAL] Extracting CNN embeddings for lazy learners (Stage {stage})...', flush=True)",
            "        train_loader_emb = DataLoader(TensorDataset(X_train_t), batch_size=32, shuffle=False)",
            "        test_loader_emb = DataLoader(TensorDataset(X_test_t), batch_size=32, shuffle=False)",
            "        train_embeds = []",
            "        test_embeds = []",
            "        with torch.no_grad():",
            "            for (inputs,) in train_loader_emb:",
            "                emb = model.get_embeddings(inputs.to(device))",
            "                train_embeds.append(emb.cpu().numpy())",
            "            for (inputs,) in test_loader_emb:",
            "                emb = model.get_embeddings(inputs.to(device))",
            "                test_embeds.append(emb.cpu().numpy())",
            "        X_train_emb = np.concatenate(train_embeds, axis=0)",
            "        X_test_emb = np.concatenate(test_embeds, axis=0)",
            "        ",
            "        rf_emb = RandomForestClassifier(n_estimators=150, criterion='entropy', max_depth=15, random_state=67, n_jobs=-1)",
            "        svm_emb = SVC(C=5.0, kernel='rbf', gamma='scale', random_state=67)",
            "        knn_emb = KNeighborsClassifier(n_neighbors=9, weights='distance', metric='cosine')",
            "        ",
            "        rf_emb.fit(X_train_emb, y_train_encoded)",
            "        svm_emb.fit(X_train_emb, y_train_encoded)",
            "        knn_emb.fit(X_train_emb, y_train_encoded)",
            "        ",
            "        rf_emb_acc = accuracy_score(y_test_encoded, rf_emb.predict(X_test_emb))",
            "        svm_emb_acc = accuracy_score(y_test_encoded, svm_emb.predict(X_test_emb))",
            "        knn_emb_acc = accuracy_score(y_test_encoded, knn_emb.predict(X_test_emb))",
            "        ",
            "        # Plot confusion matrices for ML on CNN Embeddings (using decoded target strings)",
            "        y_test_decoded = le_all.inverse_transform(y_test_encoded)",
            "        pred_rf_emb = le_all.inverse_transform(rf_emb.predict(X_test_emb))",
            "        pred_svm_emb = le_all.inverse_transform(svm_emb.predict(X_test_emb))",
            "        pred_knn_emb = le_all.inverse_transform(knn_emb.predict(X_test_emb))",
            "        plot_confusion_matrix(y_test_decoded, pred_rf_emb, f'Random Forest on CNN Embeddings (Stage {stage}) Confusion Matrix')",
            "        plot_confusion_matrix(y_test_decoded, pred_svm_emb, f'SVM on CNN Embeddings (Stage {stage}) Confusion Matrix')",
            "        plot_confusion_matrix(y_test_decoded, pred_knn_emb, f'KNN on CNN Embeddings (Stage {stage}) Confusion Matrix')",
            "        ",
            ...
            "    })",
            "    print(f'Stage {stage} Results - RF: {rf_acc:.2%} (Emb: {rf_emb_acc:.2%}), SVM: {svm_acc:.2%} (Emb: {svm_emb_acc:.2%}), KNN: {knn_acc:.2%} (Emb: {knn_emb_acc:.2%}), CNN: {cnn_acc:.2%}')",
            "    print(f'Stage {stage} Runtimes - Prep: {prep_time:.1f}s, Feat: {feat_time:.1f}s, ML: {ml_time:.1f}s, CNN: {cnn_time:.1f}s')",
            "    comparison_results.append({",
            "        'Stage': f'Stage {stage}',",
            "        'Random Forest': rf_acc,",
            "        'Random Forest (CNN Embeds)': rf_emb_acc,",
            "        'SVM (RBF)': svm_acc,",
            "        'SVM (CNN Embeds)': svm_emb_acc,",
            "        'KNN (k=9)': knn_acc,",
            "        'KNN (CNN Embeds)': knn_emb_acc,",
        ...
        add_code([
            ...
            "pct_cols = ['Random Forest', 'Random Forest (CNN Embeds)', 'SVM (RBF)', 'SVM (CNN Embeds)', 'KNN (k=9)', 'KNN (CNN Embeds)', 'CNN (Research)', 'CNN Precision', 'CNN Recall', 'CNN F1 Score']",
    ...
    add_explanation([
        ...
        "2. **Mereduksi Redundansi**: PCA memproyeksikan fitur asli ke dalam 150 komponen ortogonal yang menangkap varians data terbesar secara linier.",
    ...
    # ── CNN Embeddings (EXPERIMENTAL) ─────────────────────────────────────
    add_markdown(["## IX. [EXPERIMENTAL] Klasifikasi Menggunakan CNN Embeddings"])
    
    add_justification(
        "Pelatihan & Evaluasi Lazy Learners menggunakan CNN Embeddings",
        "Mengekstraksi fitur embedding 1280-dimensi dari model CNN yang telah dilatih, lalu melatih dan mengevaluasi model SVM, Random Forest, dan KNN menggunakan fitur tersebut.",
        "Untuk membuktikan secara eksperimental apakah representasi fitur deep learning dapat meningkatkan (smarten) akurasi pengklasifikasi tradisional dibandingkan fitur handcrafted.",
        "Interpreter mengeksekusi ekstraksi embedding di GPU/CPU dan melatih model tradisional.",
        "Model dilatih di RAM dan dievaluasi langsung di notebook.",
        "Dijalankan setelah pelatihan dan evaluasi model CNN selesai.",
        "Menggunakan DataLoader untuk mengekstrak embedding secara batch, lalu memanggil fit() and predict() pada model RF, SVM, dan KNN."
    )
    
    add_code([
        "RUN_EXPERIMENTAL = True",
        "if RUN_EXPERIMENTAL and RUN_CNN:",
        "    print(\"=== [EXPERIMENTAL] LAZY LEARNERS ON CNN EMBEDDINGS ===\")",
        "    t0_exp = time.time()",
        "    try:",
        "        import torch",
        "        import torch.nn as nn",
        ...
        "        import os",
        "        ",
        "        # 1. Inisialisasi model dan load weights",
        "        device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')",
        "        model_emb = AeroVisionEfficientNet().to(device)",
        "        if os.path.exists(checkpoint_path):",
        "            model_emb.load_state_dict(torch.load(checkpoint_path, map_location=device))",
        "            model_emb.eval()",
        "            print(\"Successfully loaded trained CNN model for embedding extraction.\")",
        "            ",
        "            # Reconstruct variables that might have been deleted in previous cell cleanup",
        "            from sklearn.preprocessing import LabelEncoder",
        "            le_emb = LabelEncoder()",
        "            y_encoded_emb = le_emb.fit_transform(labels_augmented)",
        "            X_train_img, X_test_img, y_train_encoded_emb, y_test_encoded_emb = train_test_split(",
        "                data_preprocessed, y_encoded_emb, test_size=0.2, random_state=67",
        "            )",
        "            X_train_t_emb = torch.tensor(X_train_img, dtype=torch.float32).unsqueeze(1) / 255.0",
        "            X_test_t_emb = torch.tensor(X_test_img, dtype=torch.float32).unsqueeze(1) / 255.0",
        "            ",
        "            # Re-create TensorDatasets for feature extraction",
        "            train_loader_emb = DataLoader(TensorDataset(X_train_t_emb), batch_size=32, shuffle=False)",
        "            test_loader_emb = DataLoader(TensorDataset(X_test_t_emb), batch_size=32, shuffle=False)",
        "            ",
        "            # 2. Ekstraksi Embeddings",
        "            train_embeds = []",
        "            test_embeds = []",
        "            ",
        ...
        "                    emb = model_emb.get_embeddings(inputs)",
        "                    train_embeds.append(emb.cpu().numpy())",
        "            X_train_emb = np.concatenate(train_embeds, axis=0)",
        "            ",
        ...
        "            ",
        "            # 3. Latih Lazy Learners pada CNN Embeddings",
        "            rf_emb = RandomForestClassifier(n_estimators=150, criterion='entropy', max_depth=15, random_state=67, n_jobs=-1)",
        "            svm_emb = SVC(C=5.0, kernel='rbf', gamma='scale', random_state=67)",
        "            knn_emb = KNeighborsClassifier(n_neighbors=9, weights='distance', metric='cosine')",
        "            ",
        "            print(\"\\nTraining Random Forest on CNN Embeddings...\")",
        "            rf_emb.fit(X_train_emb, y_train)",
        "            y_pred_rf = rf_emb.predict(X_test_emb)",
        "            rf_emb_acc = accuracy_score(y_test, y_pred_rf)",
        "            ",
        "            print(\"Training SVM on CNN Embeddings...\")",
        "            svm_emb.fit(X_train_emb, y_train)",
        "            y_pred_svm = svm_emb.predict(X_test_emb)",
        "            svm_emb_acc = accuracy_score(y_test, y_pred_svm)",
        "            ",
        "            print(\"Training KNN on CNN Embeddings...\")",
        "            knn_emb.fit(X_train_emb, y_train)",
        "            y_pred_knn = knn_emb.predict(X_test_emb)",
        "            knn_emb_acc = accuracy_score(y_test, y_pred_knn)",
        "            ",
        "            # Hitung akurasi tradisional untuk perbandingan langsung",
        "            rf_trad_acc = accuracy_score(y_test, rf.predict(X_test))",
        "            svm_trad_acc = accuracy_score(y_test, svm.predict(X_test))",
        "            knn_trad_acc = accuracy_score(y_test, knn.predict(X_test))",
        "            ",
        "            # 4. Cetak Perbandingan Akurasi",
        "            print(\"\\n\" + \"=\"*60)",
        "            print(\"   PERBANDINGAN AKURASI: HOG+GLCM VS CNN EMBEDDINGS\")",
        "            print(\"=\"*60)",
        "            print(f\"Random Forest - Handcrafted HOG+GLCM : {rf_trad_acc:.2%} | CNN Embeddings: {rf_emb_acc:.2%} ({'Naik' if rf_emb_acc > rf_trad_acc else 'Turun'} {abs(rf_emb_acc - rf_trad_acc):.2%})\")",
        "            print(f\"SVM (RBF)     - Handcrafted HOG+GLCM : {svm_trad_acc:.2%} | CNN Embeddings: {svm_emb_acc:.2%} ({'Naik' if svm_emb_acc > svm_trad_acc else 'Turun'} {abs(svm_emb_acc - svm_trad_acc):.2%})\")",
        "            print(f\"KNN (k=9)     - Handcrafted HOG+GLCM : {knn_trad_acc:.2%} | CNN Embeddings: {knn_emb_acc:.2%} ({'Naik' if knn_emb_acc > knn_trad_acc else 'Turun'} {abs(knn_emb_acc - knn_trad_acc):.2%})\")",
        "            print(\"=\"*60)",
        "            ",
        "            # 5. Laporan Klasifikasi & Confusion Matrix",
        "            print(\"\\n--- Random Forest on CNN Embeddings Classification Report ---\")",
        "            generateClassificationReport(y_test, y_pred_rf)",
        f"            plot_confusion_matrix(y_test, y_pred_rf, \"Random Forest on CNN Embeddings (Stage {stage_num})\")",
        "            ",
        "            print(\"\\n--- SVM on CNN Embeddings Classification Report ---\")",
        "            generateClassificationReport(y_test, y_pred_svm)",
        f"            plot_confusion_matrix(y_test, y_pred_svm, \"SVM on CNN Embeddings (Stage {stage_num})\")",
        "            ",
        "            print(\"\\n--- KNN on CNN Embeddings Classification Report ---\")",
        "            generateClassificationReport(y_test, y_pred_knn)",
        f"            plot_confusion_matrix(y_test, y_pred_knn, \"KNN on CNN Embeddings (Stage {stage_num})\")",
        "            ",
        "            # Simpan model untuk deployment/analisis",
        f"            joblib.dump(rf_emb, 'models/rf_embed_stage{stage_num}.joblib', compress=3)",
        f"            joblib.dump(svm_emb, 'models/svm_embed_stage{stage_num}.joblib', compress=3)",
        f"            joblib.dump(knn_emb, 'models/knn_embed_stage{stage_num}.joblib', compress=3)",
        "            ",
        "            # Cleanup",
        "            del model_emb, X_train_emb, X_test_emb, rf_emb, svm_emb, knn_emb",
        "            import gc",
        "            gc.collect()",
        "            if torch.cuda.is_available():",
        "                torch.cuda.empty_cache()",
        "        else:",
        "            print(f\"Model checkpoint tidak ditemukan di {checkpoint_path}.\")",
        "    except Exception as e:",
        "        print(\"Gagal menjalankan eksperimen CNN embeddings:\", e)",
    ])
    
    add_explanation([
        "Sel eksperimental ini mengekstraksi representasi fitur global (embeddings) sebesar 1280 dimensi dari model CNN EfficientNet-B0 yang telah dilatih sebelumnya, lalu melatih pengklasifikasi tradisional (Random Forest, SVM, KNN) menggunakan embeddings tersebut.",
        "",
        "**Di Balik Layar (Behind the Scenes):**",
        "1. **Ekstraksi Fitur Deep Learning**: Model CNN yang telah dilatih pada Stage ini dimuat. Masukan gambar `X_train_t` dan `X_test_t` dilewatkan melalui model hingga sebelum lapisan klasifikasi akhir (yaitu output dari `avgpool` yang diratakan menjadi vektor 1280-dimensi). Proses ekstraksi ini dijalankan secara batch untuk mencegah kegagalan memori (OOM).",
        "2. **Pelatihan Lazy Learners**: Model Random Forest, SVM, dan KNN dilatih menggunakan fitur embedding 1280-dimensi ini dengan target label kelas asli (string).",
        "3. **Perbandingan Hasil**: Akurasi pengklasifikasi yang dilatih dengan embeddings dibandingkan secara langsung dengan performa model tradisional yang dilatih menggunakan fitur handcrafted HOG+GLCM (Z-score + PCA 150). Hasilnya menunjukkan apakah fitur representasi dinamis deep learning dapat membuat model tradisional menjadi jauh lebih cerdas (smarten)."
    ])

    # ══════════════════════════════════════════════════════════════════════
    # Section X: Perbandingan Waktu Eksekusi Model
    # ══════════════════════════════════════════════════════════════════════
    # ── X. Perbandingan Waktu Eksekusi Model ────────────────────────
    add_markdown(["## X. Perbandingan Waktu Eksekusi Model [RESEARCH PURPOSES]"])
    ...
    # ══════════════════════════════════════════════════════════════════════
    # Section XI: Diskusi & Analisis Mendalam
    # ══════════════════════════════════════════════════════════════════════
    add_markdown(["## XI. Diskusi & Analisis Mendalam"])
    ...
    add_markdown([
        ...
        "",
        "Berdasarkan hasil uji coba bagian eksperimental, penggunaan **CNN Embeddings (1280-dimensi)** untuk melatih model tradisional (SVM, KNN, Random Forest) memberikan wawasan yang sangat menarik:",
        "",
        ...
        "- **SVM (RBF)** sangat unggul dalam memisahkan ruang representasi 1280-dimensi dari CNN Embeddings karena margin pemisahnya dapat menemukan batas keputusan non-linear yang sangat optimal.",
